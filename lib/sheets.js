...
//     Writing "hide" (case-insensitive) in a cell drops that case study
//     slide entirely for the user instead of ordering it.
  ...
  return rows.slice(1)
    .map((r) => {
      ...
      for (const c of CASE_KEYS) {
        ...
        if (!raw) continue;
        if (raw.toLowerCase() === 'hide') { caseHidden.push(c.key); continue; }
        const n = Number(raw);
